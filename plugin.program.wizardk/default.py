...
ADDON=xbmcaddon.Addon(id='plugin.program.wizardk')
...
EXCLUDES     = ['plugin.program.wizardk', 'script.skinshortcuts', 'resource.language.es_es']
HOME         =  xbmc.translatePath('special://home/')

    
def CATEGORIES():
    link = OPEN_URL('http://wizardk.x10host.com/wizard.txt').replace('\n','').replace('\r','')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    for name,url,iconimage,fanart,description in match:
        addDir(name,url,1,iconimage,fanart,description)
    setView('movies', 'MAIN')
	
    addDir('EN CASO DE TENER ERRORES AL ACTUALIZAR HACER INSTALACION COMPLETA, PRIMERO DAR AQUI','url',6,'','','')
...
def FRESHSTART(params):
    ...
    choice2 = xbmcgui.Dialog().yesno("Borrar todo el contenido?", '[COLOR=red]Estas seguro de borrar todo el contenido?[/COLOR]', 'Se borrara todo [B]menos[/B] el addon [COLOR=orange][B]MiWizard[/B][/COLOR]', yeslabel='Si',nolabel='No')
    if choice2 == 0:
        return
    elif choice2 == 1:
